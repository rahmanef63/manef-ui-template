...
- login di `https://gg.<YOUR_DOMAIN>/login` gagal dengan pesan:
  `Layanan login sedang tidak tersedia`
- endpoint JWKS frontend gagal:
  `https://gg.<YOUR_DOMAIN>/api/convex-auth/.well-known/jwks.json` -> `500`
- endpoint token Convex auth juga gagal ketika diuji sebelumnya:
  `https://gg.<YOUR_DOMAIN>/api/convex-auth/token` -> `500`
- kadang muncul error:
...
OPENCLAW_WORKFLOW_URL=https://ai.<YOUR_DOMAIN>/webhooks/auth/device-pending
```
...
```text
GET https://gg.<YOUR_DOMAIN>/api/convex-auth/.well-known/jwks.json -> 200 JSON
GET https://gg.<YOUR_DOMAIN>/api/convex-auth/token -> 401 if logged out, 200 JSON if logged in
```
...
1. Pastikan `manef-db` sehat dulu di `https://dbgg.<YOUR_DOMAIN>/version`.
...
- `dbgg.<YOUR_DOMAIN>` masih `502`
- route JWT bridge `gg.<YOUR_DOMAIN>/api/convex-auth/*` masih `500`
