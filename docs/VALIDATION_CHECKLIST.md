...
- [ ] `gg.<YOUR_DOMAIN>` resolve dengan benar.
- [ ] `dbgg.<YOUR_DOMAIN>` resolve dengan benar.
...
## E. Cutover frontend
- [ ] `NEXT_PUBLIC_CONVEX_URL` production diubah ke `https://dbgg.<YOUR_DOMAIN>`.
- [ ] `CONVEX_SERVER_URL` production dikosongkan atau diset sengaja bila ada kebutuhan TLS sementara.
- [ ] `HOSTED_URL` tetap `https://gg.<YOUR_DOMAIN>`.
- [ ] `NEXTAUTH_URL` tetap `https://gg.<YOUR_DOMAIN>`.
