...
- Endpoint final:
  - `https://gg.<YOUR_DOMAIN>`
  - `https://dbgg.<YOUR_DOMAIN>`
...
- Ubah `NEXT_PUBLIC_CONVEX_URL` menjadi `https://dbgg.<YOUR_DOMAIN>`.
- Kosongkan `CONVEX_SERVER_URL` kecuali memang butuh override sementara ke endpoint Convex yang TLS-nya valid.
- Pastikan `HOSTED_URL` tetap `https://gg.<YOUR_DOMAIN>`.
- Pastikan `NEXTAUTH_URL` tetap `https://gg.<YOUR_DOMAIN>`.
...
- Pastikan request frontend menuju `dbgg.<YOUR_DOMAIN>`.
