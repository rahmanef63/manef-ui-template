...
function MembersTable({
  ...
  members: UsePaginatedQueryReturnType<typeof listMembersRef>;
  viewerPermissions: NonNullable<ReturnType<typeof useViewerPermissions>>;
}) {
  const updateMember = useMutation(updateMemberRef);
  const deleteMember = useMutation(deleteMemberRef);
...
function InvitesTable({
  ...
  const deleteInvite = useMutation(deleteMemberInviteRef);
  const hasManagePermission = viewerPermissions.has("Manage Members");
  if (invites.length === 0) {
    return (
