...
import { approveDeviceRef } from "@/shared/convex/auth";
...
export async function POST(request: Request) {
  ...
  const body = JSON.parse(rawBody || "{}") as { deviceId?: string };
  if (!body.deviceId) {
    return NextResponse.json({ error: "deviceId is required" }, { status: 400 });
  }

  const result = await fetchMutation(approveDeviceRef, {
    approvedBy:
      serviceAuth.mode === "openclaw"
        ? serviceAuth.actor
        : session?.user?.email ?? session?.user?.id ?? "auth-app:system:unknown",
    deviceId: body.deviceId as Id<"authDevices">,
  });
